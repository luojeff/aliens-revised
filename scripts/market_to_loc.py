...
def offset_dep(x):
    return x['DEP_TIME'] - cml[x['ORIGIN_CITY_MARKET_ID']]['rawOffset']

def offset_arr(x):
    return x['ARR_TIME'] - cml[x['DEST_CITY_MARKET_ID']]['rawOffset']

def hhmm_to_seconds(x):
    s = str(int(x))
    s = '0' * (4 - len(s)) + s
    return 3600 * int(s[:2]) + 60 * int(s[2:])
...
for i in range(1, 9):
    ...
    flights['DEP_TIME'] = flights.apply(offset_dep, axis=1)
    flights['ARR_TIME'] = flights.apply(offset_arr, axis=1)
